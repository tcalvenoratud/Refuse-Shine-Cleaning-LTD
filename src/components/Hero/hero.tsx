...
import { ArrowRight, Sparkles, ShieldCheck, Leaf, Star, Phone } from "lucide-react"
...
export function Hero() {
  ...
  return (
    <section className="relative overflow-hidden bg-background pt-8 pb-6 md:pt-12 md:pb-8 lg:pt-16 lg:pb-10">
      {/* Decorative background glows - Optimized with transform-gpu and hidden on mobile to improve LCP/FID */}
      <div className="hidden sm:block absolute top-[-10%] right-[-10%] -z-10 size-[300px] sm:size-[500px] rounded-full bg-primary/8 dark:bg-primary/15 blur-2xl transform-gpu will-change-[filter,transform]" />
      <div className="hidden sm:block absolute bottom-[-10%] left-[-10%] -z-10 size-[300px] sm:size-[500px] rounded-full bg-accent/8 dark:bg-accent/15 blur-2xl transform-gpu will-change-[filter,transform]" />
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-8 items-center">
          ...
          >
            ...
            >
              <Sparkles className="size-3.5 text-accent dark:text-accent animate-pulse" />
              <span>Trusted Cleaning Company in the West Midlands</span>
            </motion.div>
            ...
            <motion.h1
              ...
              className="mt-6 font-heading text-3xl font-black tracking-tight text-foreground sm:text-4xl lg:text-5xl leading-[1.1]"
            >
            ...
            >
              Trusted residential and commercial cleaning services across Willenhall, Wolverhampton, Walsall and the West Midlands. Fully insured professionals with flexible online booking.
            </motion.h2>
            ...
            >
              ...
              <motion.div whileHover={shouldReduceMotion ? {} : { scale: 1.02 }} whileTap={shouldReduceMotion ? {} : { scale: 0.98 }}>
                <a href="tel:+447721714435" className="w-full sm:w-auto">
                  <Button
                    className="w-full sm:w-auto h-12 bg-accent hover:bg-accent/95 dark:bg-accent dark:hover:bg-accent/90 text-white font-bold rounded-xl px-8 shadow-lg shadow-accent/20 transition-all cursor-pointer flex items-center justify-center gap-2"
                  >
            ...
            <motion.div
              ...
              className="mt-6 pt-4 grid grid-cols-3 gap-5 sm:gap-6"
            >
          ...
          <div className="relative lg:col-span-6 flex items-center justify-center h-[350px] sm:h-[450px] md:h-[500px] lg:h-[550px] w-full mt-8 lg:mt-0">
            {/* Background glowing circle for images - Optimized with blur-xl and transform-gpu, hidden on mobile */}
            <div className="hidden sm:block absolute inset-0 m-auto -z-10 size-[80%] rounded-full bg-gradient-to-tr from-primary/10 to-accent/10 blur-xl transform-gpu will-change-[filter]" />

            {/* Sparkle animations floating around - Static / entrance fade-in only (no infinite repaints) */}
            <motion.div
              className="absolute top-[10%] left-[5%] text-accent/40 pointer-events-none"
            ...
            {/* Image 1: Larger Left/Back image - Optimised layout with stable height, entrance animations, and WebP format */}
            <div className="absolute left-4 top-4 w-[60%] h-[75%] animate-float-y">
              <motion.div
                className="relative w-full h-full rounded-[2rem] overflow-hidden border-4 border-background dark:border-card shadow-[0_20px_50px_rgba(0,0,0,0.15)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.4)] transform-gpu"
            ...
            {/* Image 2: Smaller Overlapping Right/Front image - Optimised with loading="lazy", entrance animation, and WebP format */}
            <div className="absolute right-4 bottom-4 w-[50%] h-[65%] animate-float-x">
              <motion.div
                className="relative w-full h-full rounded-[1.75rem] overflow-hidden border-4 border-background dark:border-card shadow-[0_20px_50px_rgba(0,0,0,0.2)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] transform-gpu"
